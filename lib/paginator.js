...
var Paginator = function(cfg) {
  ...
  this._content = (cfg.content) ? cfg.content : null;
  this.total = null;
};
...
Paginator.prototype._getPage = function(page, limit, cb) {
  ...
  var self = this;
  this._api._request(this._qry, function(error, body, response) {
    if (error) {
      cb(error);
    } else {
      self.total = body.meta.total;
      cb(null, (self._content) ? body[self._content] : body , page, response);
    }
  });
};
...
Paginator.prototype.getAll = function(callback) {
  ...
  this._getPage(1, this._bulkLimit, function(err, data, page, res) {
    ...
    // Get all other pages
    self.getPageRange(2, required, self._bulkLimit, function(err, data, page) {
      if (err) {
        error = true;
        callback(err);
      } else if (error) {
